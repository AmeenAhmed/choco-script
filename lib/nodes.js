...
function newStatement(node, dontAddSemicolon, checkForSuper) {
	...
	if(node.type === 'number' || 
		...
		return newFor(node);
	} else if(node.type === 'forto') {
		return newForto(node);
...
function newForto(node) {
	var ret = '';
	var varname = node.assign.var;
	var to = node.to;
	var block = node.block;

	ret += 'for(' + newAssignment(node.assign) + ';';
	...
	var statements = _.flatten(block);
	for(var i=0; i<statements.length; i++) {
		ret += newStatement(statements[i]) + ';';
	}
