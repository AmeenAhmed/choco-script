...
function newStatement(node, dontAddSemicolon, checkForSuper) {
	...
	} else if(node.type === 'ternery') {

		return newTernery(node);
	}
...
function newLambda(node) {
	...
	if(node.block) {
		var statements = _.flatten(node.block);
		for(var i=0; i<statements.length; i++) {
			// Always return the last statement
			var returnString = (i === statements.length - 1) ? "return " : "";
			
			ret += (returnString + newStatement(statements[i],false, checkForSuper) + ';');
		}
	}
...
function newTernery(node) {
	var ret = '';

	ret += newStatement(node.condition) 
			+ ' ? ' + newStatement(node.truthy)
			+ ' : ' + newStatement(node.falsy);
